...
public class EmployeeDAO {
    ...
    private static final int DEPARTMENT_NAME = 8;
    ...
    public List<Employee> findAllWithDepartmentEmployees() {
        List<Employee> employees = new ArrayList<>();
        try {
            statement = databaseConnectionManager.getConnection().prepareStatement(SELECT_ALL_ATTRIBUTES_EMPLOYEE);
            resultSet = statement.executeQuery();
            while (resultSet.next()) {
                Employee employee = toEmployeeWithDepartmentId(resultSet);
                employees.add(employee);
            }
        } catch (SQLException e) {
            this.logSQLException(e);
    ...
    private Employee toEmployeeWithDepartmentId(ResultSet row) throws SQLException {
        ...
        employee.setDepartmentStringNameString(row.getString(DEPARTMENT_NAME));
